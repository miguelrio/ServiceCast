...
def topology_setup():
    ...
    gml_file = "topologies/synthetic/topology_synthetic_1788876186136.gml"

    if Verbose.level >= 3:
        print("Network - " + gml_file)
    ...
    # server are External (i,e Internal: 0)
    network = Network.from_graph(graph, env, drop_external=False)  # use_default_weights=False,  drop_external=True)
    ...
    # 6 - clients and servers are in the GML file
    
    if Verbose.level >= 3:
        print("--- Add Servers and Clients to Network ---")

    # some servers
    # fetch names from network
    ...
    network.graphviz_to_file("synthetic.gv")
