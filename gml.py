    ...
    def filter_lines(lines):
        for line in lines:
            ...
            #except UnicodeDecodeError as err:
                #raise ValueError("input is not ASCII-encoded") from err
...
def parse_gml_lines(lines, label, destringizer):
    ...
    # general meta-data
    G.update_meta_data(graph_attr)
    # print("graph_attr = " + str(graph_attr))
    ...
    def pop_attr(dct, category, attr, i):
        try:
            ...
            raise ValueError(f"{category} #{i} has no {attr!r} attribute") from err
...
def generate_gml(G, stringizer=None):
    ...
    for attr, value in G.get_meta_data().items():     # ORIG G.graph.items():
        yield from stringize(attr, value, ignored_keys, "  ")

    # Output node data
    node_id = dict(zip(G, range(len(G))))
    ignored_keys = {"id", "label"}
    for node, attrs in G.get_node_meta_data().items():          # ORIG G.nodes.items():
        yield "  node ["
